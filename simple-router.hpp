...
public:
  ...
  void 
  sendArpRequest(const uint32_t& dst_ip);
  ...
  void
  sendIcmp(const Buffer& packet, const std::string& outIface);

  void
  sendIcmpType3(const PendingPacket& pending_packet, uint8_t type, uint8_t code);

  void
  makeIcmpt3Packet(const Buffer& frame, const Interface* iface, Buffer& icmp_packet, uint8_t type, uint8_t code);

  void
  sendIpv4(const Buffer& packet, const Interface* iface, const Buffer& dst_mac);
